...
A bridge that connects [openpilot](https://github.com/commaai/openpilot) to [Assetto Corsa Competizione](https://www.assettocorsa.it/competizione/), enabling openpilot's self-driving stack to control a car in-game.

Supports two controller modes:
- **Fanatec DD+ mode** (default): Force feedback physically turns a real Fanatec Clubsport DD+ wheelbase to openpilot's target steering angle. ACC reads the wheel position directly. Throttle and brake go through virtual pedals via uinput.
...
ACC uses shared memory for telemetry (not native UDP), so a relay script (`acc_telemetry_relay.py`) runs on the game machine to read the shared memory and forward it over UDP to the bridge.
...
- [Setup: Game Machine (Companion + Telemetry Relay)](#setup-game-machine-companion--telemetry-relay)
  - [Prerequisites](#companion-prerequisites)
  - [ACC Shared Memory Telemetry Relay](#acc-shared-memory-telemetry-relay)
  - [Fanatec DD+ Linux Driver](#fanatec-dd-linux-driver)
  - [Running the Companion](#running-the-companion)
  - [ACC Controller Settings](#acc-controller-settings)
...
- [ACC Telemetry (Shared Memory)](#acc-telemetry-shared-memory)
  - [How It Works](#telemetry-how-it-works)
...
```
  ...
  (Linux / Jetson)                            (Linux, running ACC via Proton)
 ┌──────────────────────────┐                ┌────────────────────────────┐
 │                          │                │                            │
 │  Camera                  │                │  ACC (Proton/Steam)        │
 │   │ (USB / CSI / HDMI    │                │   │                        │
 ...
 │   ├─ frames ──► modeld   │                │   ▲          ▲             │
 │   │             │        │   UDP:9000     │   │          │             │
 │   ├─ telemetry ◄─────────────────────────────── acc_telemetry_relay.py│
 │   │             ▼        │                │   │   (shared memory)      │
 │   ├─ fake CAN  plannerd  │                │   │          │             │
 │   ├─ fake panda │        │                │  companion.py             │
 │   ├─ fake DM    ▼        │                │   ├─ FFB ──►│Fanatec DD+  │
 │   │         controlsd    │                │   │  or Xbox controller    │
 │   │             │        │   UDP:5555     │   │                        │
...
**Key difference from the AC bridge**: ACC does not have a built-in UDP telemetry server. Instead, it exposes vehicle state through shared memory pages (`acpmf_physics`, `acpmf_graphics`, `acpmf_static`). The `acc_telemetry_relay.py` script reads this shared memory and forwards the data over UDP to the bridge. On Linux/Proton, the shared memory files appear in `/dev/shm/`.
...
### Game Machine
- Linux (ACC runs via Proton/Steam)
...
|---|---|---|---|
| ACC Telemetry | Game &rarr; Bridge | 9000 | Vehicle state from shared memory relay |
| Control Commands | Bridge &rarr; Game | 5555 | Steering, throttle, brake, engaged flag |

Both machines must be on the same network. If a firewall is running, open UDP port 9000 outbound on the game machine and UDP port 5555 inbound on the game machine.
...
├── acc_telemetry.py       # UDP telemetry receiver (bridge side)
├── acc_telemetry_relay.py # Shared memory reader + UDP relay (game side)
...
|---|---|---|
| `bridge.py` | Bridge | Captures camera frames, publishes to openpilot via cereal, receives ACC telemetry, reads openpilot's control output, sends commands to companion. Supports OpenCV, GStreamer, and screen capture backends. |
...
| `acc_telemetry.py` | Bridge | Receives UDP telemetry packets from the relay. Parses the 44-byte packed struct with speed, acceleration, inputs, gear, and RPM. Thread-safe with async polling mode. |
| `acc_telemetry_relay.py` | Game | Reads ACC's shared memory (Physics page) via mmap/ctypes and forwards key fields over UDP at ~100Hz. Auto-detects platform (Windows named shared memory vs Linux `/dev/shm/`). |
...
## Setup: Game Machine (Companion + Telemetry Relay)
...
ACC exposes telemetry through shared memory, not UDP. The relay script reads the Physics shared memory page and forwards it to the bridge:

```bash
# On the game machine, start the relay:
python acc_telemetry_relay.py --host <BRIDGE_MACHINE_IP> --port 9000

# With custom update rate (default: 100Hz):
python acc_telemetry_relay.py --host <BRIDGE_MACHINE_IP> --port 9000 --rate 100
```

**Linux/Proton setup**: When ACC runs under Proton, shared memory files appear in `/dev/shm/` as `acpmf_physics`, `acpmf_graphics`, and `acpmf_static`. The relay auto-detects the platform and uses the appropriate method.

**Windows setup**: The relay uses Windows named shared memory via `mmap(tagname=...)`.

The relay must be running before starting the bridge. It reads the Physics page which contains speed, acceleration, throttle, brake, steering angle, gear, and RPM.
...
1. In ACC Settings &rarr; Controls, select the Fanatec device for the steering axis
2. Assign throttle to "AC Bridge Virtual Pedals" &rarr; ABS_Z axis
3. Assign brake to "AC Bridge Virtual Pedals" &rarr; ABS_RZ axis
4. Set steering linearity and deadzone to minimum (the bridge handles all processing)

#### Xbox Mode

1. In ACC Settings &rarr; Controls, set input method to **Gamepad**
...
Network:
  ...
  --telemetry-port PORT     UDP port for ACC telemetry (default: 9000)
...
**1. Start the telemetry relay on the game machine:**
...
**3. Start ACC, load a session (practice, race, etc.)**
...
Required:
  --game-host HOST          IP of the game machine running ACC

Network:
  --companion-port PORT     UDP port for control commands (default: 5555)
  --telemetry-port PORT     UDP port to receive ACC telemetry (default: 9000)
...
docker build -f tools/acc_bridge/Dockerfile.l4t -t acc-bridge-l4t .

# Or with a different DeepStream base version
docker build -f tools/acc_bridge/Dockerfile.l4t \
...
| `IMAGE_TAG` | `acc-bridge-l4t` | Docker image tag |
...
ACC is a realistic GT racing simulator, so steering tuning should be similar to the AC bridge defaults. The cars tend to have high downforce and responsive steering.
...
## ACC Telemetry (Shared Memory)

### How It Works {#telemetry-how-it-works}

Unlike Assetto Corsa (which has a built-in UDP telemetry server), ACC uses shared memory for exposing vehicle state. The data flow is:

```
ACC (game process)
  │
  ├─ writes to shared memory (Physics page)
  │    Linux/Proton: /dev/shm/acpmf_physics
  │    Windows: named shared memory "Local\\acpmf_physics"
  │
  ▼
acc_telemetry_relay.py (on game machine)
  │
  ├─ reads Physics struct via mmap/ctypes at ~100Hz
  ├─ packs key fields into 44-byte UDP packet
  │
  ▼
acc_telemetry.py (on bridge machine, UDP port 9000)
  │
  └─ provides thread-safe access to latest telemetry state
```

### Telemetry Fields

The relay packs these fields from the Physics shared memory page:

| Field | Type | Description |
|---|---|---|
| `speed_kmh` | float | Vehicle speed in km/h |
| `speed_ms` | float | Vehicle speed in m/s (speed_kmh / 3.6) |
| `acc_g_x` | float | Lateral acceleration (g) |
| `acc_g_y` | float | Vertical acceleration (g) |
| `acc_g_z` | float | Longitudinal acceleration (g) |
| `gas` | float | Throttle position [0, 1] |
| `brake` | float | Brake position [0, 1] |
| `steer_angle` | float | Steering angle (degrees) |
| `gear` | int32 | Gear (0=reverse, 1=neutral, 2+=forward) |
| `rpm` | float | Engine RPM |
| `turbo_boost` | float | Turbo boost pressure |
...
```bash
# On the bridge machine:
python acc_telemetry.py --port 9000
```

Start ACC, load a session, and with the relay running you should see:

```
Listening for ACC telemetry on UDP port 9000...
ACC telemetry connected (from 192.168.1.100)
...
┌─ ACC Telemetry ───────────────────────────────────────────────────────────┐
│ UDP from acc_telemetry_relay.py (speed, g-forces)                        │
...
| ACC telemetry | ~async | Receive UDP telemetry from relay |
...
**"Listening for ACC telemetry..." but no data received**
- Verify `acc_telemetry_relay.py` is running on the game machine
- Check that ACC is running and in a session (not in menus)
- Verify the relay is configured with the correct bridge machine IP
- On Linux/Proton: verify shared memory files exist: `ls /dev/shm/acpmf_*`
- Check network connectivity: `ping <BRIDGE_MACHINE_IP>` from the game machine
- Ensure UDP port 9000 is not blocked by a firewall
...
**Virtual Xbox controller not detected by ACC** (Xbox mode)
- Verify uinput is loaded: `ls /dev/uinput`
- Run companion as root if permission denied: `sudo python companion.py --xbox`
- Check that ACC input method is set to "Gamepad" in settings
...
**ACC shared memory not found (Linux/Proton)**
- ACC must be running and in a session for the shared memory to be created
- Check `/dev/shm/` for files starting with `acpmf_`
- If running under Proton, the shared memory prefix may differ — check `ls /dev/shm/ | grep -i acc`
- Try restarting ACC and entering a session before starting the relay
...
- Use `--window-title "Assetto Corsa Competizione"` to target the game window specifically
