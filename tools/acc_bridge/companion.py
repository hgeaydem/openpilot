#!/usr/bin/env python3
"""
ACC companion script - runs on the GAME MACHINE.

Receives steering/throttle/brake commands from the openpilot bridge over UDP,
and emits them on a virtual Xbox 360 controller via uinput. ACC (native or
via Proton) reads this controller as standard gamepad input.

Usage:
  python companion.py --listen-port 5555

  In ACC settings, set input method to "Gamepad" and the game will
...
def main():
    parser = argparse.ArgumentParser(description='ACC Bridge companion (game machine side)')
    ...
    print("  Make sure ACC input is set to 'Gamepad'")
