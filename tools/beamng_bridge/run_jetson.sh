#!/bin/bash
# Launch the BeamNG.drive bridge container on NVIDIA Jetson.
#
# Usage:
#   ./run_jetson.sh --game-host 192.168.1.100
#   ./run_jetson.sh --game-host 192.168.1.100 --cam-source v4l2-mjpeg --cam-device /dev/video0
#   ./run_jetson.sh --game-host 192.168.1.100 --cam-source rtsp --cam-device rtsp://192.168.1.100:8554/game
#
# Environment variables:
#   CAMERA_DEVICE  - camera device path (default: /dev/video0)
#   IMAGE_TAG      - Docker image tag (default: beamng-bridge-l4t)
...
IMAGE_TAG="${IMAGE_TAG:-beamng-bridge-l4t}"
...
echo "Starting BeamNG.drive bridge on Jetson..."
...
exec docker run -it --rm \
    ...
    -e SIMULATION=1 \
    -e NOSENSOR=1 \
    -v "$SCRIPT_DIR/tweak.cfg:/openpilot/tools/beamng_bridge/tweak.cfg" \
    "$IMAGE_TAG" \
